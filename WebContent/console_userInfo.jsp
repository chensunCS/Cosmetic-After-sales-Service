...
<body class="container">
    ...
    <%
    	...
    	//获取本次查询的用户数量
    	int currentUserNum = userInfos.size();
    	//定义注册用户的数量
    	int allUserNum = 0;
    	//定义查询用户数量的SQL语句
    	String countUserInfo = "select count(user_id) as count from user_info";
    	//获取注册用户的数量
    	allUserNum = selectDB.countTable(countUserInfo);
    	
    	//定义页面的开始，结束位置
    	int pageBeginIndex = (pageIndex - (pageIndex % 5)) + 1;
    	int pageEndIndex = 0;
    	//判断是否已经到了末尾
    	if(allUserNum <= dataNum) {
    		pageEndIndex = pageBeginIndex;
    	}
    	else if(dataNum * (pageBeginIndex % 5) + 5 * dataNum <= allUserNum) {
    		//如果没有到末尾，则自动增加5页
    		pageEndIndex = pageBeginIndex + 5;
    	}
    	else {
    		//判断是否刚好除整
    		if(allUserNum % dataNum == 0) {
        		//如果到了末尾
        		pageEndIndex = pageBeginIndex + Math.abs(allUserNum - pageBeginIndex * dataNum) / dataNum;
    		}
    		//如果总数没有除整
    		else {
        		//如果到了末尾
        		pageEndIndex = pageBeginIndex + Math.abs(allUserNum - pageBeginIndex * dataNum) / dataNum + 1;
    		}
    	}
    %>
    ...
    <header class="page-header hidden-xs">
        <h1>我的控制台<small>用户注册表格(user_info) #<%=pageIndex %></small></h1>
    </header>
    ...
    <div class="row">
        ...
        <div class="col-sm-12">
        	<p>
        		<span class="label label-warning">注册人数： <%=allUserNum + " 人 / 400人" %></span>
        	</p>
        </div>
    	<!-- 显示一共有多少条信息，用进度条显示 -->
        <div class="col-sm-12">
        	<div class="progress">
				...
				<div class="progress-bar progress-bar-warning progress-bar-striped active" role="progressbar" 
					 style="width: <%="" + ((float)allUserNum/400.0 * 100) + "%"%>">
					<span style="color: black"><%="" + ((float)allUserNum/400.0 * 100) + "%"%></span>
				</div>
			</div>
        </div>
        <!--显示下拉菜单按钮-->
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu" role="menu" aria-labelledby="dropdownMenu1">
                        <li role="presentation">
                        	<a role="menuitem" tabindex="-1" href="<%="./console_userInfo.jsp?pageIndex=1&dataNum=" + 10%>" >每页10条</a>
                        </li>
                        <li role="presentation">
                        	<a role="menuitem" tabindex="-1" href="<%="./console_userInfo.jsp?pageIndex=1&dataNum=" + 20%>" >每页20条</a>
                        </li>
                        <li role="presentation">
                        	<a role="menuitem" tabindex="-1" href="<%="./console_userInfo.jsp?pageIndex=1&dataNum=" + 50%>">每页50条</a>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <table class="table table-striped table-hover">
                    ...
                    %>
                    	<tr>
                            ...
                            <td><%=userInfo.getUserPresentProduct() %></td>
                            <td><%=userInfo.getUserSkinAttr() %></td>
                            <td><%=userInfo.getUserSkinProblem() %></td>
                        </tr>
                    <%
                    	}
                    %>
        ...
        <div class="col-sm-12 text-center">
            <nav>
                <ul class="pager">
                	...
                	%>
	                	<!-- 显示上一页的标识 -->
	                    <li><a href="<%="./console_userInfo.jsp?pageIndex=" + (pageIndex-1) +"&dataNum=" + dataNum %>">上一页</a></li>
                    <%
                		}
                    %>
                    ...
                    %>
	                    <li><a href="<%="./console_userInfo.jsp?pageIndex=" + i +"&dataNum=" + dataNum %>"><%=i %></a></li>
                    <%
                    	}
                    %>
                    ...
                    <%
                    	//如果不是尾页
                    	if(currentUserNum == dataNum && pageIndex * dataNum < allUserNum) {
                    %>
                    	<!-- 显示下一页的标识 -->
                    	<li><a href="<%="./console_userInfo.jsp?pageIndex=" + (pageIndex+1) +"&dataNum=" + dataNum %>">下一页</a></li>
                    <%
                    	}
                    %>
                </ul>
    ...
    <div class="row">
    	<!--显示下载文档的按钮-->
        <div class="col-sm-12 text-center">
       		<button type="button" class="btn btn-success" onclick="exportExcel('<%=Source.userInfoTable %>', '<%=Source.queryUserInfo%>')">
            	下载成Excel文档查看
